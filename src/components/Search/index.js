...
import { useAllPairsInImpossible, useAllTokensInImpossible } from '../../contexts/GlobalData'
...
export const Search = ({ small = false }) => {
  let allTokens = useAllTokensInImpossible()
  const allTokenData = useAllTokenData()

  let allPairs = useAllPairsInImpossible()
        ...
        <Input
          ...
          placeholder={
            small
              ...
              ? 'Search...'
              : below470
              ? 'Search Impossible Finance...'
              : below700
              ? 'Search pairs and tokens...'
              : 'Search Impossible Finance pairs and tokens...'
          }
          value={value}
          onChange={(e) => {
            ...
            filteredPairList.slice(0, pairsShown).map((pair) => {
              ...
              return (
                <BasicLink to={'/pair/' + pair.id} key={pair.id} onClick={onDismiss}>
                  <MenuItem>
                    <DoubleTokenLogo a0={pair?.token0} a1={pair?.token1} margin={true} />
